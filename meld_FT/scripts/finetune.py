...
MELD_DIR = CURRENT_DIR.parent
DATASET_DIR = Path(MELD_DIR) / "datasets"
...
if not os.path.exists(os.path.join(MELD_DIR, "model_args")):
    os.mkdir(os.path.join(MELD_DIR, "model_args"))

train_file = os.path.join(MELD_DIR, "model_args", f"""{train_dataset_name.split(".")[0].split("train")[0]}{BASE_MODEL.split("/")[1]}.json""")
...
with open(os.path.join(OUTPUT_DIR, f"""meld_results_{NB_EPOCHS}.pickle"""), 'wb') as fh:
